...
export function AboutPage(): React.ReactElement {
  ...
  const name = language === "uz" ? data?.name_uz : (data?.name_ru || data?.name_uz);
  const about = language === "uz" ? data?.about_uz : (data?.about_ru || data?.about_uz);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="about-page__hero">
            {data.logo ? (
              <img src={data.logo} alt={name || "Logo"} className="about-page__logo" />
            ) : (
            ...
            {data.has_delivery && (
              <div className="about-page__badge">
                {language === "uz" ? "🚗 Yetkazib berish mavjud" : "🚗 Доставка доступна"}
              </div>
            )}
          </div>
          ...
          {/* Kontakt ma'lumotlar */}
          {(data.contact_number || data.contact_email || data.telegram_account) && (
            <div className="about-page__section">
              <h3 className="about-page__section-title">
              ...
              <div className="about-page__contacts">
                {data.contact_number && (
                  <a href={`tel:${data.contact_number}`} className="about-page__contact-item">
                    <div className="about-page__contact-icon about-page__contact-icon--phone">📞</div>
                    <div className="about-page__contact-info">
                      <span className="about-page__contact-label">
                        {language === "uz" ? "Telefon" : "Телефон"}
                      </span>
                      <span className="about-page__contact-value">{data.contact_number}</span>
                    </div>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="about-page__contact-arrow">
                      <path d="M7.5 15L12.5 10L7.5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </a>
                )}
                {data.contact_email && (
                  <a href={`mailto:${data.contact_email}`} className="about-page__contact-item">
                    <div className="about-page__contact-icon about-page__contact-icon--email">✉️</div>
                    <div className="about-page__contact-info">
                      <span className="about-page__contact-label">Email</span>
                      <span className="about-page__contact-value">{data.contact_email}</span>
                    </div>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="about-page__contact-arrow">
                      <path d="M7.5 15L12.5 10L7.5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                ...
                {data.telegram_account && (
                  ...
                  >
                    <div className="about-page__contact-icon about-page__contact-icon--tg">✈️</div>
                    <div className="about-page__contact-info">
                      <span className="about-page__contact-label">Telegram</span>
                      <span className="about-page__contact-value">{data.telegram_account}</span>
                    </div>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="about-page__contact-arrow">
                      <path d="M7.5 15L12.5 10L7.5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
            ...
            <div className="about-page__section">
              ...
              <div className="about-page__map-card">
                ...
                <div className="about-page__map-info">
                  {data.adress_name && (
                    <span className="about-page__map-address">{data.adress_name}</span>
                  )}
                  {data.latitude && data.longitude && (
                    ...
                    >
                      {language === "uz" ? "Xaritada ko'rish →" : "Смотреть на карте →"}
                    </a>
                  )}
                </div>
              </div>
        ...
        <div className="about-page__empty">
          <p>{language === "uz" ? "Ma'lumot topilmadi" : "Информация не найдена"}</p>
        </div>
      )}
    </div>
  );
